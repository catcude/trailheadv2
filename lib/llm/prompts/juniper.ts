 ...
 * Voice + §Safety, docs/v2 §GuidepostPhilosophy). The LLM's ONLY job here is
 * to soften an authored line toward a calibrated tone. It must not add
 * content, questions, advice, or resources; it must not change meaning; and
 * it never selects flow.
 *
...
Never present a feeling, pattern, or trait as who the student is — it's information, not identity. Leave interpretation with the student: don't assign hope, motivation, or direction, and don't add certainty a line doesn't have. Steady beats cheerful.`;
